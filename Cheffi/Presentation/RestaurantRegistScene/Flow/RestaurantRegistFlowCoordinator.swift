...
final class RestaurantRegistFlowCoordinator {
    ...
    func start() {
        steps
            .sink { [weak self] step in
                ...
                switch step {
                case .restaurantRegistSearch: return pushRestaurantRegistSearch()
                case .restaurantRegistCompose: return pushRestaurantRegistCompose()
                case .restaurantInfoCompose: return pushRestaurantInfoCompose()
                }
            }
    ...
    func pushRestaurantInfoCompose() {
        let reducer = dependencies.makeRestaurantInfoComposeReducer(steps: steps)
        let vc = dependencies.makeRestaurantInfoComposeViewController(reducer: reducer)
        navigationController?.pushViewController(vc, animated: true)
    }
}
